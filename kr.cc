...
validation(const int nread, const RW_SET read_set[], const int tx_begin, const int tx_end)
{
  ...
  list<DONE_XACT>::iterator xitr;
  for (xitr = DoneXact.begin(); xitr != DoneXact.end(); xitr++)
  {
    if (xitr->tx_id == tx_begin)
    {
      xitr++;
      break;
    }
  ...
  // Do Validation
  for (;xitr != DoneXact.end() && xitr->tx_id <= tx_end; xitr++)
  {
    for (int write_id = 0; write_id < xitr->nwrite; write_id++)
    {
      for (int read_id = 0; read_id < nread; read_id++)
      {
        if (xitr->write_set[write_id] == read_set[read_id]) //conflict
        {
          return false;
        }
      }
